...
export class MyReportsService {
    ...
    public getReportData(token, entity, type) {
        let httpHeaders = new HttpHeaders({
            'x-auth-token': token
        })
        return this.http.get(AppConfigs.api_url + '/unnati/api/v1/reports/getMonthlyOrQuarterReportPdf?reportType=' + type + '&schoolName=' + entity.name + '&' + entity.entityId, { headers: httpHeaders })
    }
    public share(data) {
